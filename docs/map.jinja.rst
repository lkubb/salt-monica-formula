...
We will see a quick setup to configure the ``monica`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``monica`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``monica`` formulas under ``/srv/salt/monica/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/monica/parameters/dns:domain/
    ln -s dns:domain /srv/salt/monica/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/monica/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/monica/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``monica`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``monica`` formulas under ``/srv/salt/monica/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/monica/parameters/roles
...
We create a configuration for the role ``monica/server`` in ``/srv/salt/monica/parameters/roles/monica/server.yaml``:
...
We create another configuration for the role ``monica/client`` in ``/srv/salt/monica/parameters/roles/monica/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles monica/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles monica/client
...
.. code-block:: console
    ...
    mv /srv/salt/monica/parameters/dns:domain/  '/srv/salt/monica/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-monica-as-json:
      test.nop:
        - name: {{ monica | json }}
...
Here is an example based on `template-formula/monica/config/file.sls`_:
    ...
    monica-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='monica-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ monica.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``monica`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ monica['winner'] }}
...
.. _template-formula/monica/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/monica/config/file.sls
